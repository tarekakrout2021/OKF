...
def plot_quantiles(
    x, ax=None, Q=100, showmeans=False, plot_mean=False, means_args=None, **kwargs
):
    if plot_mean:
        showmeans = True  # backward compatibility
    if ax is None:
        ax = Axes(1, 1)[0]
    m = np.mean(x)
...
class Axes:
    def __init__(self, N, W=2, axsize=(5, 3.5), grid=1, fontsize=13):
        ...
        self.axs = plt.subplots(
            self.H, self.W, figsize=(self.W * axsize[0], self.H * axsize[1])
        )[1]
...
def compare_with_ref(
    dd, y, x, ref=None, x_ord=None, hue=None, sort_hue=True, ax=None, font=14, **kwargs
):
    if ax is None:
        ax = Axes(1, 1, axsize=(6, 4))[0]
    if x_ord is None:
        x_ord = sorted(np.unique(dd[x]))
    if ref is None:
        ref = x_ord[0]
    ...
    **kwargs,
    ...
    for a in range(nf):
        ...
        if box:
            ...
            sns.lineplot(
                data=d, x=xx, y=y, hue=hue, hue_order=curr_hues, ax=ax, ci=ci, **kwargs
            )
        if relabel:
