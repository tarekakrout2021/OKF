...
def get_trainable_data(X, Z):
    ...
    return [x.values.astype(np.double) for x in X], [
        z.values.astype(np.double) for z in Z
    ]
...
def display_data(X, Z):
    ...
    axs.labs(
        ...
        f"Lengths distribution over {len(X):d} trajectories",
    )
...
def simulate_data(
    ...
    at_sigma=15,
    noise_r=2,
    noise_t=1.5 * np.pi / 180,
    fpath="data/simple_lidar_data.pkl",
):
    X, Z = [], []
